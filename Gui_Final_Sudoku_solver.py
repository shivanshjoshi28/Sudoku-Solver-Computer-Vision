...
window=tk.Tk() # setting tkinter window
window.title("Sudoku Solver With AI ") #giving the title
canvas = Canvas(window, width = 650, height = 400)  # to insert the game logo ( taken from stackOverflow to insert image in tkinter )
...
# to get the corner array ( to find the maximum area rectangle )
def get_corners(img):
...
# to get the sudoku area ( the required image for model testing )
...
# to get vertical and horizontal lines of sudoku
...
#creating a mask for getting the numbers inside the sudoku and empty space in sudoku
def create_grid_mask(vertical, horizontal):
    ...
    def draw_lines(im, pts):   # code taken from stack overflow for the Hough lines transformation
        im = np.copy(im)
        pts = np.squeeze(pts)
        for r, theta in pts:
            a = np.cos(theta)
...
#extract each digits in sorted order with x and y pos sorted
...
#adding the border to make it clear to the model prediction
...
#dividing the transformed image in 81 x 81 subimage (i.e each numbers inside the sudoku )
...
#adding zeros image ( means totally black if the area is less than a particular threshold )
...
# to convert the image to array ( i. e the unsolved puzzle.... Prediction will be done here )
...
# this is to put the solution . i.e solved puzzle to each block inside sudoku
def put_solution(img_arr, soln_arr, unsolved_arr):
    ...
    for solution, unsolved, img in paired:
        ...
        posx = img_w // 4 - img_w // 11    # randomly generated values by manipulation and self checking
        posy = 2 * img_h // 3 + img_h // 11  # randomly generated values by manipulation and self checking
...
# to join all the small images with solution ( to form 9x9 square .i.e. final sudoku )
...
# this is to stitch the warped image ( with solution ) to the original frame image
...
# this is the function to solve the sudoku with the input image
...
def solve_webcam(path,debug=True):
    ...
    while True:
        ...
        try:
            ...
            puzzle = img_to_array(digits_with_zeros, model_image_size)
            print(puzzle)
            if np.sum(puzzle) == 0:
...
# Function for opening the video file and solve it ( same as that of webcam acess )
...
# this is to solve the image of sudoku if possible
def image_opener():
    filename = filedialog.askopenfilename(filetypes=(("All files", "*.*")    # this opening image code taken from StackOverflow
                                                     # ,("Template files", "*.tplate")
                                                       , ("HTML files", "*.html;*.htm")))
    solve_photo(filename)

# this is to sove the image using the webcam or the i.p camera (whose server IP address is passed )
def Gui_webcam():
    if(t1.get()==""):
        path=0
    else:
